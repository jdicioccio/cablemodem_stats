...
impl OutputFormatter for CricketFormatter {
    fn format(info: &ChannelInfo) -> Result<String, String> {
        ...
        for (channel_id, channel) in info.downstream_info.iter() {
            iovec.push(format!("{}_ds.freq_mhz:{}", channel_id, channel.freq_mhz));
            iovec.push(format!(
                "{}_ds.power_dbmv:{}",
                channel_id, channel.power_dbmv
            ));
            iovec.push(format!("{}_ds.snr_db:{}", channel_id, channel.snr_db));
            iovec.push(format!(
                "{}_ds.corrected_errs:{}",
            ...
            iovec.push(format!("{}_ds.lock_status:{}", channel_id, locked));

            match channel.uncorrected_errs_delta {
                Some(delta) => iovec.push(format!("{}_ds.uncorrected_errs:{}", channel_id, delta)),
                None => (),
            };
            match channel.corrected_errs_delta {
                Some(delta) => {
                    iovec.push(format!("{}_ds.corrected_errs_delta:{}", channel_id, delta))
                }
                None => (),
            };
        }
        ...
        for (channel_id, channel) in info.upstream_info.iter() {
            iovec.push(format!("{}_us.sym_rate:{}", channel_id, channel.sym_rate));
            iovec.push(format!("{}_us.freq_mhz:{}", channel_id, channel.freq_mhz));
            iovec.push(format!("{}_us.sym_rate:{}", channel_id, channel.sym_rate));
            iovec.push(format!(
                "{}_us.power_dbmv:{}",
                channel_id, channel.power_dbmv
            ...
            iovec.push(format!("{}_us.lock_status:{}", channel_id, locked));
        }
